...
export function EditVideogamePage() {
  const { updateVideogame, getVideogame } = useVideogames()
    ...
    <div>
      <div className='form-content'>
        <div className='title-form'>
          <h1 className='create-videogame-title'>Update <span className='span-title'>Videogame</span></h1>
        </div>
        <Formik
          ...
          onSubmit={async (values, actions) => {
            await updateVideogame(params.id, values)
            navigate(`/viewVideogame/${params.id}`)
          }}
          enableReinitialize
        >
          {({ handleSubmit }) => (
